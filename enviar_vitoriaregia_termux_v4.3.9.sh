...
[ -d "$EXTRACT_DIR/substituir-no-repositorio" ] && SOURCE_DIR="$EXTRACT_DIR/substituir-no-repositorio"
...
git add .
git commit -m "Atualização Vitória Régia ${APP_VERSION}" >> "$LOG_FILE" 2>&1 || true
