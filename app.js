(function () {
  ...
  const VERSION = '4.3.1-mysql-organizado';
  const VERSION_LABEL = `v${VERSION}`;
  const STORE_KEY = 'vitoriaRegiaStore.v340';
  const SESSION_KEY = 'currentUser';
  const LEGACY_KEYS = ['vitoriaRegiaStore.v330', 'vitoriaRegiaStore.v320', 'vitoriaRegiaStore.v3'];
  ...
  const pageMap = [
    { id: 'dashboard', icon: '🏠', label: 'Dashboard', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'moradores', icon: '🏢', label: 'Moradores', roles: ['sindico', 'subsindico', 'admin', 'owner', 'administracao'] },
    { id: 'usuarios', icon: '👥', label: 'Usuários', roles: ['sindico', 'admin', 'owner', 'administracao'] },
    { id: 'aprovacoes', icon: '✅', label: 'Aprovações', roles: ['sindico', 'subsindico', 'admin', 'owner', 'administracao'] },
    { id: 'encomendas', icon: '📦', label: 'Encomendas', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'visitantes', icon: '🪪', label: 'Visitantes', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'reservas', icon: '📅', label: 'Reservas', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'comunicados', icon: '📣', label: 'Comunicados', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'servicos', icon: '🧹', label: 'Solicitações', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'ajuda', icon: '❔', label: 'Ajuda', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'aplicativos', icon: '📱', label: 'Aplicativos', roles: ['morador', 'portaria', 'sindico', 'subsindico', 'admin', 'owner', 'zeladoria', 'limpeza', 'administracao'] },
    { id: 'emergencias', icon: '🛟', label: 'Emergências', roles: ['portaria', 'sindico', 'subsindico', 'admin', 'owner', 'administracao'] },
    { id: 'premium', icon: '⭐', label: 'Central premium', roles: ['admin', 'owner'] },
    { id: 'configuracoes', icon: '⚙️', label: 'Configurações', roles: ['sindico', 'admin', 'owner', 'administracao'] },
    { id: 'backups', icon: '💾', label: 'Backup', roles: ['sindico', 'admin', 'owner', 'administracao'] }
  ];
  ...
  const defaultStore = {
    ...
    settings: {
      ...
      profilePermissions: {
        morador: { dashboard:true, encomendas:true, visitantes:true, reservas:true, comunicados:true, aplicativos:true },
        portaria: { dashboard:true, encomendas:true, visitantes:true, comunicados:true, aplicativos:true, emergencias:true },
        subsindico: { dashboard:true, moradores:true, encomendas:true, visitantes:true, reservas:true, comunicados:true, servicos:true, aplicativos:true, emergencias:true, backups:true },
        limpeza: { dashboard:true, servicos:true, comunicados:true, ajuda:true },
        zeladoria: { dashboard:true, servicos:true, comunicados:true, ajuda:true }
      }
    },
    users: [
  ...
  function normalizeRole(value) {
    ...
    if (raw.includes('limpeza')) return 'limpeza';
    if (raw.includes('zelador') || raw.includes('zeladoria')) return 'zeladoria';
    if (raw === 'administracao' || raw === 'administração') return 'administracao';
  ...
  function mergeStore(raw) {
    ...
    for (const key of ['users','residents','packages','visitors','recurringVisitors','serviceRequests','bookings','notices','emergencies','notifications','cloudFiles','pendingRegistrations','passwordResetRequests','appInstallRequests','units','notificationOutbox','updateRequests','backups','supportTickets','logs']) {
  ...
  function renderPage(page, user) {
    const renderers = { dashboard, moradores, usuarios, aprovacoes, encomendas, visitantes, reservas, comunicados, servicos, ajuda, aplicativos, emergencias, premium, configuracoes, backups };
  ...
  function dashboard(user) {
    ...
    const actionSets = {
      ...
      owner: [
        ...
        action('servicos','🧹','Solicitações','Aprovar e enviar equipe'),
  ...
  function usuarios() {
    const s = loadStore();
    const staffRoles = ['administracao','portaria','zeladoria','limpeza'];
    const staffRows = s.users.filter(u => staffRoles.includes(normalizeRole(u.role)) || ['admin','owner','sindico','subsindico','portaria'].includes(normalizeRole(u.role)));
    return `<div class="cadastro-layout">
      <section class="card full-span"><div class="section-title"><h3>Usuários e funcionários</h3><span class="badge ok">Sem unidade residencial</span></div>
        <p class="note">Funcionários não precisam de unidade. Cadastre por setor: administração, portaria, zeladoria ou limpeza. Moradores continuam no menu Moradores.</p>
        <div class="tab-strip"><span class="active">Cadastrar funcionário</span><span>Consultar usuários</span><span>Permissões</span></div>
      </section>
      <section class="card"><div class="section-title"><h3>Cadastrar funcionário</h3><span class="badge">Setor interno</span></div><form id="userForm" class="form-grid">
        <label class="field"><span>Nome completo</span><input name="name" required placeholder="Ex.: Ana da Portaria"></label>
        <label class="field"><span>Usuário de acesso</span><input name="username" required placeholder="Ex.: portaria01"></label>
        <label class="field"><span>E-mail</span><input name="email" type="email" placeholder="para senha temporária e avisos"></label>
        <label class="field"><span>WhatsApp</span><input name="phone" placeholder="Ex.: 5561999999999"></label>
        <label class="field"><span>Telegram / Chat ID</span><input name="telegram" placeholder="@usuario ou Chat ID"></label>
        <label class="field"><span>Senha inicial</span><input name="password" type="password" required placeholder="Senha temporária"></label>
        <label class="field"><span>Setor</span><select name="role"><option value="administracao">Administração</option><option value="portaria">Portaria</option><option value="zeladoria">Zeladoria</option><option value="limpeza">Limpeza</option><option value="sindico">Síndico</option><option value="subsindico">Subsíndico</option><option value="admin">Administrador</option><option value="owner">Proprietário do sistema</option></select></label>
        <label class="field"><span>Turno, se aplicável</span><select name="shift"><option>Não se aplica</option><option>Manhã</option><option>Tarde</option><option>Noite</option><option>Plantão 12x36</option></select></label>
        <button class="btn primary">Salvar funcionário</button>
      </form></section>
      <section class="card"><div class="section-title"><h3>Consultar usuários</h3><span class="badge">${s.users.length}</span></div>
        ${table(s.users, [{label:'Nome',value:'name'}, {label:'Usuário',value:'username'}, {label:'Perfil/Setor',value:r => roleLabels[normalizeRole(r.role)] || r.role || '-'}, {label:'Telegram',value:r => r.telegram || r.telegramChatId || '-'}, {label:'Ativo',value:r => r.active === false ? 'Não' : 'Sim'}], row => `<div class="row-actions"><button class="btn ghost small" data-toggle-user="${row.id}">${row.active === false ? 'Ativar' : 'Desativar'}</button></div>`)}
      </section>
      <section class="card full-span"><div class="section-title"><h3>Gerenciar permissões por perfil</h3><span class="badge">O que cada perfil vê</span></div>${permissionsEditor(s)}</section>
    </div>`;
  }

  function permissionsEditor(s) {
    const roles = ['morador','portaria','subsindico'];
    const pages = ['dashboard','encomendas','visitantes','reservas','comunicados','servicos','ajuda','aplicativos','emergencias','moradores','usuarios','backups'];
  ...
  function servicos(user) {
    const s = loadStore();
    s.serviceRequests = Array.isArray(s.serviceRequests) ? s.serviceRequests : [];
    const rows = user.role === 'morador' && user.apartment ? s.serviceRequests.filter(r => String(r.apartment||'').toLowerCase() === String(user.apartment).toLowerCase()) : s.serviceRequests;
    const canManage = ['sindico','subsindico','admin','owner','portaria','zeladoria','limpeza'].includes(user.role);
    const tipos = ['Limpeza em área comum','Vazamento de água','Queda de fase de energia','Lâmpada queimada','Portão/porta com defeito','Elevador','Barulho/uso indevido','Outro problema'];
    return `<div class="grid cols-2">
      <section class="card"><div class="section-title"><h3>Nova reclamação ou solicitação</h3><span class="badge">Morador</span></div>
        <p class="note">Use este menu para avisar a administração sobre limpeza, vazamentos, queda de fase de energia ou outro problema do condomínio.</p>
        <form id="serviceForm" class="form-grid">
          <label class="field"><span>Tipo de problema</span><select name="type">${tipos.map(t=>`<option>${escapeHtml(t)}</option>`).join('')}</select></label>
          <label class="field"><span>Local ou unidade</span><input name="apartment" value="${escapeHtml(user.apartment || '')}" placeholder="Ex.: garagem, hall, 305"></label>
          <label class="field"><span>Prioridade</span><select name="priority"><option>Normal</option><option>Alta</option><option>Urgente</option></select></label>
          <label class="field full-span"><span>Descreva de forma simples</span><textarea name="description" required placeholder="Ex.: corredor do 3º andar está sujo / queda de fase no bloco B / vazamento próximo à garagem"></textarea></label>
          <button class="btn primary">Enviar para análise</button>
        </form>
      </section>
      <section class="card"><div class="section-title"><h3>Como funciona</h3><span class="badge ok">Fluxo organizado</span></div>
        <div class="flow-steps"><div><b>1</b><span>Morador registra o problema</span></div><div><b>2</b><span>Síndico ou administração aprova</span></div><div><b>3</b><span>Equipe de limpeza/zeladoria recebe</span></div><div><b>4</b><span>Conclui e registra solução</span></div></div>
      </section>
      <section class="card full-span"><div class="section-title"><h3>Solicitações registradas</h3><span class="badge">${rows.length}</span></div>
        ${table(rows, [{label:'Tipo',value:'type'}, {label:'Local',value:'apartment'}, {label:'Prioridade',value:'priority'}, {label:'Status',value:'status'}, {label:'Responsável',value:r=>r.assignedTo||'-'}, {label:'Data',value:r=>formatDate(r.createdAt)}], row => canManage ? `<div class="row-actions"><button class="btn ok small" data-approve-service="${row.id}">Aprovar</button><button class="btn secondary small" data-assign-service="${row.id}">Enviar equipe</button><button class="btn ghost small" data-close-service="${row.id}">Concluir</button></div>` : '')}
      </section>
    </div>`;
  }
  ...
  function emergencias(user) {
    ...
    const activeRows = rows.filter(e => !['resolvido','resetado','cancelado'].includes(String(e.status || '').toLowerCase()));
    const cards = [
      ['🚑','Emergência médica','Atendimento de saúde ou mal-estar grave'],
      ['💧','Vazamento de água','Risco de dano, infiltração ou alagamento'],
      ['🔥','Vazamento de gás','Risco crítico. Ventile e evite faíscas'],
      ['🛗','Preso no elevador','Mostra assistência cadastrada nas configurações'],
      ['⚡','Queda de fase de energia','Falta parcial de energia ou risco elétrico'],
      ['🛡️','Segurança / invasão','Situação suspeita ou risco de segurança']
    ];
    return `<div class="emergency-page">
      <section class="card full-span"><div class="section-title"><h3>Central de emergências</h3><button class="btn danger small" data-open-emergency>Acionar emergência</button></div>
        <p class="note">Escolha o tipo certo de ocorrência. O sistema identifica o usuário logado, registra o local e avisa primeiro síndico, administração e portaria.</p>
        <div class="emergency-type-grid">${cards.map(c=>`<button class="emergency-type-card" data-open-emergency-type="${escapeHtml(c[1])}"><span>${c[0]}</span><strong>${escapeHtml(c[1])}</strong><small>${escapeHtml(c[2])}</small></button>`).join('')}</div>
      </section>
      <section class="card"><div class="section-title"><h3>Ocorrências abertas</h3><span class="badge warn">${activeRows.length}</span></div>${activeRows.length ? activeRows.map(e=>`<div class="emergency-row"><div><strong>${escapeHtml(e.type)}</strong><p>${escapeHtml(e.description || 'Sem descrição')}</p><small>${escapeHtml(e.apartment || 'Local não informado')} • ${escapeHtml(formatDate(e.createdAt))}</small></div><span class="badge warn">${escapeHtml(e.status || 'aguardando')}</span></div>`).join('') : '<p class="note">Nenhuma emergência aberta no momento.</p>'}</section>
      <section class="card"><div class="section-title"><h3>Histórico e ações</h3><span class="badge">${rows.length}</span></div>
        ${table(rows, [{label:'Tipo',value:'type'}, {label:'Local',value:'apartment'}, {label:'Relato',value:'description'}, {label:'Status',value:'status'}, {label:'Data',value:r=>formatDate(r.createdAt)}], row => `<div class="row-actions"><button class="btn ok small" data-confirm-emergency="${row.id}">Confirmar</button><button class="btn warn small" data-reset-emergency="${row.id}">Resetar</button><button class="btn ghost small" data-resolve-emergency="${row.id}">Resolver</button></div>`)}
      </section>
    </div>`;
  }
  ...
  function configuracoes(user) {
    const s = loadStore();
    const onlyOwner = ['owner','admin'].includes(user.role);
    return `<div class="settings-premium">
      <section class="card full-span"><div class="section-title"><h3>Configurações do sistema</h3><span class="badge">${VERSION_LABEL}</span></div>
        <p class="note">Configurações ficam separadas dos cadastros. Cadastros de pessoas ficam nos menus Moradores e Usuários; aqui ficam regras, canais, aparência, emergência, apps, banco e backup.</p>
        <div class="settings-tabs"><a href="#cfg-geral">Geral</a><a href="#cfg-canais">Canais</a><a href="#cfg-emergencia">Emergência</a><a href="#cfg-apps">Apps</a><a href="#cfg-banco">Banco/Backup</a></div>
      </section>
      <section id="cfg-geral" class="card"><div class="section-title"><h3>Geral e aparência</h3><span class="badge">Visual</span></div><form id="settingsForm" class="form-grid">
        ...
        <label class="field"><span>Imagem de fundo do prédio</span><input name="buildingBackground" value="${escapeHtml(s.settings.buildingBackground)}" placeholder="URL ou assets/condominio-fachada.png"></label>
        ...
        <button class="btn primary">Salvar geral</button>
      </form></section>
      <section class="card"><div class="section-title"><h3>Unidades residenciais</h3><span class="badge">Cadastro de unidades</span></div><form id="unitToolsForm" class="form-grid">
        ...
        <p class="note">Esta configuração cria unidades residenciais. Funcionários são cadastrados sem unidade no menu Usuários.</p>
        <button class="btn primary">Gerar unidades</button>
      </form></section>
      <section id="cfg-canais" class="card full-span"><div class="section-title"><h3>Canais de notificação</h3><span class="badge">Links clicáveis</span></div>
        <div class="channel-shortcuts"><a href="https://telegram.org/apps" target="_blank" rel="noopener">📲 Baixar Telegram</a><a href="https://wa.me/5561996071663" target="_blank" rel="noopener">💬 Testar WhatsApp</a><a href="mailto:bmedeiros1987@gmail.com?subject=Suporte%20Vitória%20Régia" target="_blank" rel="noopener">✉️ Enviar e-mail</a></div>
        <form id="channelsForm" class="form-grid">${channelsFormFields(s)}<button class="btn primary">Salvar canais</button></form>
      </section>
      <section id="cfg-emergencia" class="card"><div class="section-title"><h3>Emergência e assistência</h3><span class="badge danger">Crítico</span></div><form id="emergencySettingsForm" class="form-grid">
        <label class="field"><span>Assistência do elevador</span><input name="elevatorCompany" value="${escapeHtml((s.settings.emergency && s.settings.emergency.elevatorCompany) || '')}" placeholder="Nome da empresa"></label>
        <label class="field"><span>Telefone/WhatsApp do elevador</span><input name="elevatorPhone" value="${escapeHtml((s.settings.emergency && s.settings.emergency.elevatorPhone) || '')}" placeholder="Ex.: 5561999999999"></label>
        <label class="field full-span"><span>Orientação para preso no elevador</span><textarea name="elevatorGuidance" placeholder="Ex.: manter calma, não forçar portas, aguardar assistência.">${escapeHtml((s.settings.emergency && s.settings.emergency.elevatorGuidance) || '')}</textarea></label>
        <button class="btn primary">Salvar emergência</button>
      </form></section>
      <section id="cfg-apps" class="card"><div class="section-title"><h3>Aplicativos e acesso rápido</h3><span class="badge">Usuários</span></div><p class="note">Os links para Telegram, WhatsApp e e-mail ficam clicáveis na aba Apps e Ajuda. Use este bloco para orientar moradores e equipes.</p><button class="btn secondary" data-page="aplicativos">Abrir Apps</button></section>
      <section id="cfg-banco" class="card"><div class="section-title"><h3>Banco, atualização e backup</h3><span class="badge ok">${onlyOwner ? 'Admin' : 'Restrito'}</span></div>
        ${onlyOwner ? `<form id="updateForm" class="form-grid"><label class="field"><span>Arquivo ZIP da atualização</span><input name="updateZip" type="file" accept=".zip"></label><label class="field"><span>Hook de publicação, opcional</span><input name="renderDeployHook" value="${escapeHtml(s.settings.renderDeployHook)}" placeholder="URL segura de publicação" /></label><button class="btn primary">Registrar atualização</button><button class="btn secondary" type="button" id="triggerDeploy">Acionar publicação</button></form>` : `<p class="note">Apenas o administrador do sistema vê atualização, banco e publicação.</p>`}
      </section>
      <section class="card"><div class="section-title"><h3>Nuvem de imagens</h3><span class="badge">${escapeHtml(s.settings.cloudMode)}</span></div><form id="cloudForm" class="form-grid"><label class="field"><span>Modo de armazenamento</span><select name="cloudMode"><option value="local" ${s.settings.cloudMode==='local'?'selected':''}>Local/backup do sistema</option><option value="cloudinary" ${s.settings.cloudMode==='cloudinary'?'selected':''}>Nuvem/API externa</option></select></label><label class="field"><span>Nome/identificador da nuvem</span><input name="cloudName" value="${escapeHtml(s.settings.cloudName)}" placeholder="Ex.: vitoria-regia"></label><button class="btn primary">Salvar nuvem</button></form></section>
      <section class="card"><div class="section-title"><h3>Segurança</h3><span class="badge warn">Acesso</span></div><p class="note">A tela inicial não exibe perfil nem unidade. O sistema identifica tudo pelo usuário e senha.</p><button class="btn ghost" id="lockFirstAccess">Bloquear criação automática de primeiro admin</button></section>
    </div>`;
  }
  ...
  function bindCommonActions(page, user) {
    ...
    document.querySelectorAll('[data-open-emergency-type]').forEach(btn => btn.addEventListener('click', () => openEmergencyModal(user, btn.dataset.openEmergencyType)));
    document.querySelectorAll('[data-approve-service]').forEach(btn => btn.addEventListener('click', () => updateServiceRequest(btn.dataset.approveService, 'aprovada pelo síndico/administração')));
    document.querySelectorAll('[data-assign-service]').forEach(btn => btn.addEventListener('click', () => updateServiceRequest(btn.dataset.assignService, 'encaminhada para equipe', true)));
    document.querySelectorAll('[data-close-service]').forEach(btn => btn.addEventListener('click', () => updateServiceRequest(btn.dataset.closeService, 'concluída')));
  ...
  async function bindForms(page, user) {
    ...
    serviceForm?.addEventListener('submit', ev => { ev.preventDefault(); const d = Object.fromEntries(new FormData(ev.currentTarget).entries()); const s = loadStore(); s.serviceRequests = s.serviceRequests || []; const item = { id:id('service'), requester:user.name || user.username, requesterRole:user.role, status:'aguardando análise', ...d, createdAt:now() }; s.serviceRequests.unshift(item); addNotification(s, 'Nova solicitação do morador', `${d.type} - ${d.apartment || 'local não informado'}.`, 'Síndico/Administração'); dispatchChannelNotification(s, { kind:'service', title:'Nova solicitação', body:`${d.type} - ${d.apartment || '-'}. ${d.description || ''}`, apartment:d.apartment || 'Administração' }); logAction('servicos', `Solicitação registrada: ${d.type}.`); saveStore(s); toast('Solicitação enviada para análise.'); renderApp('servicos'); });
    ...
    emergencySettingsForm?.addEventListener('submit', ev => { ev.preventDefault(); const d = Object.fromEntries(new FormData(ev.currentTarget).entries()); const s = loadStore(); s.settings.emergency = { ...(s.settings.emergency || {}), ...d }; logAction('configuracoes', 'Configurações de emergência atualizadas.'); saveStore(s); toast('Configurações de emergência salvas.'); renderApp('configuracoes'); });
    ...
    permissionsForm?.addEventListener('submit', ev => {
      ...
      const roles = ['morador','portaria','subsindico']; const pages = ['dashboard','encomendas','visitantes','reservas','comunicados','servicos','ajuda','aplicativos','emergencias','moradores','usuarios','backups'];
  ...
  function updateServiceRequest(serviceId, status, assign) {
    const s = loadStore();
    s.serviceRequests = s.serviceRequests || [];
    const row = s.serviceRequests.find(r => r.id === serviceId);
    if (!row) return;
    row.status = status;
    row.updatedAt = now();
    if (assign) row.assignedTo = row.type && String(row.type).toLowerCase().includes('limpeza') ? 'Equipe de limpeza' : 'Zeladoria/Manutenção';
    addNotification(s, 'Solicitação atualizada', `${row.type} - ${status}.`, row.apartment || 'Todos');
    dispatchChannelNotification(s, { kind:'service', title:'Solicitação atualizada', body:`${row.type}: ${status}`, apartment:row.apartment || 'Todos' });
    logAction('servicos', `Solicitação ${row.type} alterada para ${status}.`);
    saveStore(s); toast(`Solicitação ${status}.`); renderApp('servicos');
  }
  ...
  function openEmergencyModal(user, preselectedType) {
    removeDuplicatePanicButtons();
    const s0 = loadStore();
    const emergencyCfg = s0.settings.emergency || {};
    const wrap = document.createElement('div');
    wrap.className = 'modal-backdrop';
    const types = ['Emergência médica','Vazamento de água','Vazamento de gás','Preso no elevador','Queda de fase de energia','Segurança / invasão','Outro problema urgente'];
    wrap.innerHTML = `<div class="modal emergency-modal"><div class="modal-head"><h3>🚨 Acionar emergência</h3><button class="close" type="button">×</button></div>
      <p class="note">Confirme apenas se precisar de ajuda agora. O alerta será registrado com seu usuário e enviado para síndico/administração e portaria.</p>
      <form id="emergencyForm" class="form-grid">
        <label class="field full-span"><span>Tipo de emergência</span><select name="type">${types.map(t=>`<option ${t===preselectedType?'selected':''}>${escapeHtml(t)}</option>`).join('')}</select></label>
        <div class="emergency-choice-preview full-span"><strong>Escolha com calma</strong><span>Toque acidental é evitado com confirmação final.</span></div>
        <label class="field"><span>Local ou unidade</span><input name="apartment" value="${escapeHtml(user.apartment || '')}" required placeholder="Ex.: 305, garagem, elevador social"></label>
        <label class="field"><span>Seu nome</span><input value="${escapeHtml(user.name || user.username || '')}" readonly></label>
        <label class="field full-span"><span>Descrição breve</span><textarea name="description" placeholder="Descreva o que está acontecendo"></textarea></label>
        <div class="elevator-assistance full-span"><strong>Assistência do elevador</strong><p>${escapeHtml(emergencyCfg.elevatorCompany || 'Cadastre a empresa em Configurações > Emergência.')}</p>${emergencyCfg.elevatorPhone ? `<a href="https://wa.me/${escapeHtml(String(emergencyCfg.elevatorPhone).replace(/\D/g,''))}" target="_blank" rel="noopener">Chamar assistência pelo WhatsApp</a>` : ''}<small>${escapeHtml(emergencyCfg.elevatorGuidance || 'Se estiver preso no elevador: mantenha a calma, não force as portas e aguarde orientação da portaria/assistência.')}</small></div>
        <label class="check-line full-span"><input type="checkbox" name="confirm" required> Confirmo que preciso acionar síndico e portaria agora.</label>
        <button class="btn danger">Enviar emergência</button><p class="note full-span">Em risco imediato à vida, acione também os serviços oficiais de emergência.</p>
      </form></div>`;
    document.body.appendChild(wrap);
    wrap.querySelector('.close').onclick = () => wrap.remove();
    wrap.addEventListener('click', ev => { if (ev.target === wrap) wrap.remove(); });
    const select = wrap.querySelector('select[name="type"]');
    const elevatorBox = wrap.querySelector('.elevator-assistance');
    const syncElevator = () => { elevatorBox.style.display = select.value === 'Preso no elevador' ? 'block' : 'none'; };
    select.addEventListener('change', syncElevator); syncElevator();
    wrap.querySelector('#emergencyForm').addEventListener('submit', ev => {
      ...
      notifyBrowser('Emergência acionada', `${item.type} - ${item.apartment}`); wrap.remove(); toast('Alerta enviado para síndico e portaria.'); renderApp('emergencias');
    });
  }
