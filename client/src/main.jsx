...
const sensitive = new Set(['SENDGRID_API_KEY','SMTP_PASS','TELEGRAM_BOT_TOKEN','TELEGRAM_WEBHOOK_SECRET','WHATSAPP_ACCESS_TOKEN','WHATSAPP_API_TOKEN','UPDATE_GITHUB_TOKEN','DATABASE_URL','JWT_SECRET','BANK_CLIENT_SECRET','BANK_API_TOKEN','VAPID_PRIVATE_KEY','KIOSK_PORTARIA_PIN','KIOSK_PORTARIA_PIN_HASH']);
...
function initialForms(){ return {
  login:{ email:'', password:'' }, register:{ name:'', gender:'nao_informado', email:'', phone:'', whatsapp_phone:'', telegram_username:'', telegram_chat_id:'', unit:'', document:'', role:'morador' }, forgot:{ email:'' },
...
async function request(path, opts={}){
  ...
  const { raw:_raw, blob:_blob, timeoutMs:_timeoutMs, ...fetchOptions } = opts;
  try{
    const res = await fetch(API + path, { ...fetchOptions, signal:opts.signal || controller?.signal, headers:{ ...headers, ...(token ? { Authorization:'Bearer '+token } : {}) } });
    ...
    if(e?.name === 'AbortError') throw new Error('O servidor demorou para responder. Verifique a conexão e tente novamente; o sistema bloqueou o envio duplicado.');
    throw e;
...
function App(){
  ...
  useEffect(() => {
    const configTabs = ['aparencia','notificacoes','email','telegram','whatsapp','banco','emergencia','condominio','areas','apps','documentos','atualizacoes','auditoria'];
    if(active === 'configuracoes' && configTabs.includes(sub)) setConfigTab(sub);
  }, [active, sub]);

  function setForm(group, patch){ setForms(f => ({ ...f, [group]:{ ...f[group], ...patch } })); }
  useEffect(()=>()=>{ if(toastTimerRef.current) clearTimeout(toastTimerRef.current); },[]);
  function notify(message, fail=false){
    if(toastTimerRef.current) clearTimeout(toastTimerRef.current);
    setToast({ message:String(message || (fail ? 'Não foi possível concluir a operação.' : 'Operação concluída.')), fail:Boolean(fail) });
    document.body.classList.toggle('shake', Boolean(fail));
    toastTimerRef.current=setTimeout(()=>{ setToast(null); document.body.classList.remove('shake'); }, 3800);
  }
  ...
  async function action(path, body, message, method='POST'){
    try {
      ...
      notify(message);
      // A tela deve responder assim que o servidor confirmar a gravação; a atualização ampla roda sem bloquear o modal.
      void loadAll();
      return true;
...
function CameraCaptureModal({type='package',onClose,onCapture,notify}){
  ...
  useEffect(()=>{
    ...
    async function start(){
      try{
        if(!window.isSecureContext) throw new Error('A câmera exige conexão HTTPS segura.');
        if(!navigator.mediaDevices?.getUserMedia) throw new Error('Este navegador/APK não oferece câmera ao vivo.');
        const attempts=[
          { video:{ facingMode:{ ideal:'environment' }, width:{ ideal:1280 }, height:{ ideal:720 } }, audio:false },
          { video:{ facingMode:'environment' }, audio:false },
          { video:true, audio:false }
        ];
        let media=null, lastError=null;
        for(const constraints of attempts){
          try{ media=await navigator.mediaDevices.getUserMedia(constraints); break; }
          catch(e){ lastError=e; if(['NotAllowedError','SecurityError'].includes(e?.name)) break; }
        }
        if(!media) throw lastError || new Error('Nenhuma câmera disponível.');
        ...
        const denied=['NotAllowedError','SecurityError','PermissionDeniedError'].includes(e?.name);
        const message=denied ? 'Permissão de câmera negada. Libere Câmera nas configurações do Android e reabra o aplicativo.' : (e?.message || 'Câmera indisponível neste aparelho.');
        setStatus(message+' Você também pode usar a câmera padrão abaixo.');
        notify?.(message, true);
      }
    }
    start();
  ...
  function stop(){ try{ streamRef.current?.getTracks?.().forEach(t=>t.stop()); streamRef.current=null; }catch{} }
  ...
  return <div className="cameraReaderOverlay"><div className="cameraReaderBox"><div className="cameraReaderHead"><div><b>{type==='package'?'Leitura automática da etiqueta':'Leitura automática'}</b><small>{status}</small></div><button type="button" onClick={()=>{stop(); onClose();}}><X/></button></div><div className="cameraReaderStage"><video ref={videoRef} playsInline muted autoPlay/><div className="cameraGuide"><span>Alinhe a etiqueta aqui</span></div></div><canvas ref={canvasRef} style={{display:'none'}}/><div className="cameraReaderActions"><button type="button" className="secondaryAction" onClick={()=>{stop(); onClose();}}>Cancelar</button><label className="fileButton cameraFallbackButton"><ScanLine/> Usar câmera padrão<input type="file" accept="image/*" capture="environment" onChange={async e=>{ const file=e.target.files?.[0]; if(file){ stop(); await onCapture(file,type,{codes:[]}); } }}/></label><button type="button" className="confirmAction" onClick={capture}><Camera/> Capturar etiqueta</button></div><small className="cameraReaderTip">Se a câmera ao vivo não abrir, use “câmera padrão”. No APK, permita Câmera em Configurações do Android / Apps / Vitória Régia Portaria / Permissões.</small>{detectedCodes.length>0&&<div className="scannerDetectedBox"><b>QR/código detectado</b>{detectedCodes.map(c=><small key={c.rawValue}>{c.format}: {c.rawValue}</small>)}</div>}</div></div>;
}


function TelegramLinkBox({ entity='me', id='', initialUrl='', compact=false }){
  const [url,setUrl]=useState(initialUrl || '');
  const [status,setStatus]=useState('');
  async function generate(){
    setStatus('Gerando link de vínculo...');
    try{
      const r=await request('/api/telegram/link-token',{method:'POST',body:JSON.stringify({entity,id}),headers:{'Content-Type':'application/json'}});
      setUrl(r.url || '');
      setStatus('Link gerado. Abra no celular do usuário e toque em START / INICIAR.');
    }catch(e){ setStatus(e.message || 'Não foi possível gerar o link.'); }
  }
  async function copy(){
    if(!url) return;
    try{ await navigator.clipboard?.writeText(url); setStatus('Link copiado.'); }catch{ setStatus('Copie o link manualmente.'); }
  }
  return <div className={compact?'telegramLinkMini':'telegramLinkBox'}>
    {!compact&&<><b>Vincular Telegram</b><small>O usuário deve abrir o link e tocar em START / INICIAR para o sistema salvar o Chat ID automaticamente.</small></>}
    <div className="telegramLinkActions"><button type="button" className="secondaryAction" onClick={generate}><Send/> Gerar link Telegram</button>{url&&<><a className="buttonlike secondary" href={url} target="_blank" rel="noreferrer"><MessageCircle/> Abrir bot</a><button type="button" className="secondaryAction" onClick={copy}>Copiar link</button></>}</div>
    {url&&<small className="telegramLinkUrl">{url}</small>}
...
function LoginPage({ forms,setForm,mode,setMode,doLogin,err,setShowPass,showPass,action,settings }){
  ...
  const submitRegister = async (e) => {
    ...
    try {
      ...
      setRegisterStatus({ ok:true, text:r?.message || 'Solicitação enviada para aprovação. Você receberá retorno nos canais cadastrados.', telegramLinkUrl:r?.telegram_link_url || r?.request?.telegram_link_url || '' });
    } catch (ex) {
  ...
  return <div className="loginPage buildingLogin redesignedLogin cleanAccess">
    ...
    <section className="loginCard premiumLoginCard accessCard">
      ...
      {mode==='register' && <form className="registerForm" onSubmit={submitRegister}>
        ...
        {registerStatus && <div className={registerStatus.ok?'registerStatus success':'registerStatus error'}><b>{registerStatus.ok?'Solicitação enviada':'Não foi possível enviar'}</b><small>{registerStatus.text}</small>{registerStatus.ok&&registerStatus.telegramLinkUrl&&<div className="telegramLinkActions"><a className="buttonlike secondary" href={registerStatus.telegramLinkUrl} target="_blank" rel="noreferrer"><MessageCircle/> Vincular meu Telegram</a><small>Abra o bot e toque em START / INICIAR para o sistema preencher seu Chat ID automaticamente.</small></div>}</div>}
...
function Moradores({data,forms,setForm,action,openConfirm,settings,del,loadAll}){
  ...
  return <div className="stack residentsPage"><form className="formGrid premiumForm" onSubmit={e=>{e.preventDefault(); openConfirm(f.id?'Confirmar alteração de morador':'Confirmar cadastro de morador', summary, ()=>action(f.id?`/api/residents/${f.id}`:'/api/residents', f, f.id?'Morador atualizado':'Morador cadastrado', f.id?'PUT':'POST'));}}>
    ...
    <label>Chat ID Telegram<input value={f.telegram_chat_id||''} onChange={e=>setForm('resident',{telegram_chat_id:e.target.value})} placeholder="ID numérico após /start no bot"/><small>Usado para receber mensagens do sistema.</small></label>{f.id&&<TelegramLinkBox entity="resident" id={f.id}/>}
...
function Usuarios({data,forms,setForm,action,openConfirm,del,loadAll,session}){
  ...
  return <div className="stack usersPage"><form className="formGrid premiumForm" onSubmit={e=>{e.preventDefault(); openConfirm(f.id?'Confirmar alteração de usuário':'Confirmar cadastro de usuário',{Nome:f.name,'E-mail':f.email,Perfil:roleLabel(f.role),Unidade:f.unit},()=>action(f.id?`/api/users/${f.id}`:'/api/users', f, f.id?'Usuário atualizado':'Usuário cadastrado', f.id?'PUT':'POST'));}}>
    ...
    <label>Chat ID Telegram<input value={f.telegram_chat_id||''} onChange={e=>setForm('user',{telegram_chat_id:e.target.value})} placeholder="ID numérico após /start no bot"/><small>Usado para receber mensagens do sistema.</small></label>{f.id&&<TelegramLinkBox entity="user" id={f.id}/>}
...
function ErrorLogs({action,isAdminReserved}){
  ...
  async function load(){
    ...
    try{
      const rows = await request('/api/error-logs');
      setLogs(Array.isArray(rows) ? rows : []);
    } catch(e){
      setLogs([]);
      setError(e?.message || 'Não foi possível carregar os logs técnicos.');
    } finally { setLoading(false); }
  }
  useEffect(()=>{load().catch(()=>null)},[]);
  return <div className="stack errorLogPanel">
    <div className="auditToolbar"><button type="button" className="secondaryAction" onClick={load}><RefreshCcw/> Atualizar logs</button>{isAdminReserved&&<button type="button" className="dangerAction" onClick={()=>action('/api/error-logs',{},'Logs limpos','DELETE').then(load)}><Trash2/> Limpar logs</button>}</div>
    {loading&&<small>Carregando...</small>}
    {error&&<div className="noticeBox warn"><b>Logs técnicos restritos</b><small>{error}</small></div>}
    <div className="logCards">{logs.map(l=><article className="logCard" key={l.id || `${l.path}-${l.created_at}`}><div><Status ok={false}>Erro</Status><b>{l.message || 'Erro registrado'}</b><small>{l.method || '-'} {l.path || '-'}</small></div><small>{l.actor || 'sistema'} · {l.created_at ? new Date(l.created_at).toLocaleString('pt-BR') : '-'}</small></article>)}{!logs.length&&!loading&&!error&&<div className="noticeBox ok"><b>Nenhum erro registrado</b><small>O sistema não possui logs recentes de erro.</small></div>}</div>
  </div>;
}
...
function AuditPage({data,action,loadAll,isAdminReserved}){
  ...
  const filteredAudit = useMemo(()=>{
    ...
    return auditRows.filter(a => `${a.actor||''} ${a.action||''} ${a.entity||''} ${a.created_at||''}`.toLowerCase().includes(q));
  },[auditRows,filter]);
  const grouped=useMemo(()=>{
    const m={};
    for(const item of filteredAudit){
      const key=String(item?.action || 'Ação sem categoria');
      (m[key] ||= []).push(item || {});
    }
    return m;
  },[filteredAudit]);
  const lastAudit = auditRows[0]?.created_at ? new Date(auditRows[0].created_at).toLocaleString('pt-BR') : 'sem registros recentes';
  return <div className="stack auditPremium auditSafe">
    <div className="auditHero"><History/><div><h3>Auditoria do sistema</h3><small>Tela recriada com proteção contra dados ausentes, erro de permissão e retorno inesperado do backend.</small></div></div>
    <div className="metricStrip aligned auditMetrics"><div className="metric"><span><History/></span><div><b>{auditRows.length}</b><small>ações registradas</small></div></div><div className="metric"><span><Bell/></span><div><b>{notificationRows.length}</b><small>notificações recentes</small></div></div><div className="metric"><span><Clock/></span><div><b>{lastAudit}</b><small>última ação</small></div></div></div>
    <SettingCard title="Filtros e atualização" icon={<Search/>}><div className="formGrid"><label>Buscar na auditoria<input value={filter} onChange={e=>setFilter(e.target.value)} placeholder="Usuário, ação, entidade ou data"/></label><button type="button" onClick={()=>loadAll?.()}><RefreshCcw/> Atualizar auditoria</button></div></SettingCard>
    <SettingCard title="Logs de erro" icon={<Activity/>}><ErrorLogs action={action} isAdminReserved={isAdminReserved}/></SettingCard>
    <SettingCard title="Log de notificações" icon={<Bell/>}><Table rows={notificationRows} render={n=><><td><b>{n.title || 'Notificação'}</b><small>{n.body || n.message || '-'}</small></td><td><Code>{channelNames[n.channel]||n.channel||'Sistema'}</Code><small>{n.action_url || ''}</small></td><td><Status ok={['lida','enviada','registrada'].includes(String(n.status||'').toLowerCase())}>{n.status || 'registrada'}</Status><small>{date(n.created_at)}</small></td></>}/></SettingCard>
    {Object.entries(grouped).length ? Object.entries(grouped).map(([groupAction,rows])=><div className="subpanel auditGroup" key={groupAction}><h3>{groupAction}</h3><Table rows={rows} render={a=><><td><b>{a.actor || 'sistema'}</b><small>{a.entity || '-'}</small></td><td>{date(a.created_at)}</td></>}/></div>) : <div className="noticeBox ok"><b>Nenhum item de auditoria encontrado</b><small>Não há registros para o filtro atual ou seu perfil não possui registros liberados no momento.</small></div>}
  </div>;
}
