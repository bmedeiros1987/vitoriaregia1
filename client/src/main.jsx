...
const VERSION = import.meta.env.VITE_APP_VERSION || 'Vitória Régia Pro v12.6.2';
...
function initialForms(){ return {
  ...
  profile:{ name:'', email:'', phone:'', whatsapp_phone:'', telegram_chat_id:'', unit:'', document:'', vehicle:'', notification_preferences:{ app:true, browser:true, email:true, telegram:true, whatsapp:false }, password:'' }, householdMember:{ name:'', email:'', phone:'', whatsapp_phone:'', telegram_chat_id:'', document:'' },
  resident:{ id:'', name:'', unit:'', phone:'', whatsapp_phone:'', email:'', telegram_chat_id:'', document:'', vehicle:'', vehicle_model:'', vehicle_plate:'', pet_name:'', notes:'', resident_tags:{}, notification_preferences:{ app:true, browser:true, email:true, telegram:true, whatsapp:false } },
  user:{ id:'', name:'', email:'', password:'', role:'morador', user_type:'morador', unit:'', phone:'', whatsapp_phone:'', telegram_chat_id:'', resident_id:'', active:true, notification_preferences:{ app:true, browser:true, email:true, telegram:true, whatsapp:false } },
    ...
    <section className="loginCard premiumLoginCard accessCard">
      ...
      {mode==='register' && <form className="registerForm" onSubmit={submitRegister}>
        ...
        <div className="formGrid registerGrid">
          ...
          {channels.telegram && <div className="noticeBox ok"><b>Telegram do condomínio já configurado</b><small>As notificações usam o chat padrão cadastrado pelo síndico. Não é necessário informar Chat ID no cadastro.</small></div>}
        </div>
  ...
  return <div className="stack">
    ...
    <Table rows={data.packages} render={p=><>
      ...
      <td className="actions packageActions">
        {isResident ? <><button onClick={()=>action(`/api/packages/${p.id}/preference`,{delivery_preference:'receber_elevador'},'Preferência enviada à portaria em serviço')}>Receber pelo elevador</button><button onClick={()=>action(`/api/packages/${p.id}/preference`,{delivery_preference:'retirar_portaria'},'Preferência enviada à portaria em serviço')}>Retirar na portaria</button><button className="confirmAction" onClick={()=>confirmDelivery(p,'resident')}>Confirmo recebimento</button></> : <button className="confirmAction" onClick={()=>confirmDelivery(p,'staff')}>Indicar entregue</button>}
...
function NotifyTests({forms,setForm,action,data}){
  ...
  const defaultTelegramChat = cfg.telegram?.chatDefaultRaw || forms.settings?.TELEGRAM_CHAT_ID || DEFAULT_TELEGRAM_CHAT_ID;
  const target = channel==='telegram' ? defaultTelegramChat : channel==='whatsapp' ? (f.phone || f.to || '') : (f.to || '');
  const payload = { channel, to:target, chat_id:channel==='telegram' ? defaultTelegramChat : (f.chat_id || target), phone:f.phone || target, subject:f.subject || 'Teste Vitória Régia', message:f.message || 'Mensagem de teste do Sistema Vitória Régia.' };
  return <SettingCard title="Teste de notificações" icon={<Send/>}>
    ...
    {channel==='telegram' && <div className="noticeBox ok"><b>Telegram padrão</b><small>O teste será enviado para o chat cadastrado nas notificações: {defaultTelegramChat || DEFAULT_TELEGRAM_CHAT_ID}. Não é solicitado Chat ID nesta tela.</small></div>}
...
function NotificationSettings(props){ const {forms,setForm,action}=props; const s=forms.settings; const telegramChat=s.TELEGRAM_CHAT_ID || DEFAULT_TELEGRAM_CHAT_ID; return <div className="stack"><SettingCard title="Canais liberados" icon={<Bell/>}><div className="channels"><label><input type="checkbox" checked={bool(s.ENABLE_EMAIL,true)} onChange={e=>setForm('settings',{ENABLE_EMAIL:String(e.target.checked)})}/>E-mail</label><label><input type="checkbox" checked={bool(s.ENABLE_WHATSAPP,false)} onChange={e=>setForm('settings',{ENABLE_WHATSAPP:String(e.target.checked)})}/>WhatsApp</label><label><input type="checkbox" checked={bool(s.ENABLE_TELEGRAM,false)||bool(s.TELEGRAM_ENABLED,false)} onChange={e=>setForm('settings',{ENABLE_TELEGRAM:String(e.target.checked),TELEGRAM_ENABLED:String(e.target.checked)})}/>Telegram</label><label><input type="checkbox" checked={bool(s.ENABLE_BROWSER_PUSH,true)} onChange={e=>setForm('settings',{ENABLE_BROWSER_PUSH:String(e.target.checked)})}/>Navegador</label></div><div className="formGrid"><label>Chat ID padrão do Telegram<input value={telegramChat} onChange={e=>setForm('settings',{TELEGRAM_CHAT_ID:e.target.value,TELEGRAM_TEST_CHAT_ID:e.target.value})} placeholder={DEFAULT_TELEGRAM_CHAT_ID}/><small>Usado automaticamente por todo o sistema. Altere aqui para trocar o destino global do Telegram.</small></label></div><button className="saveConfig" onClick={()=>saveSettings(forms,action)}><Save/> Salvar canais e Chat ID</button></SettingCard><NotifyTests {...props}/></div>; }
