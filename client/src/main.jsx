...
const VERSION = import.meta.env.VITE_APP_VERSION || 'Vitória Régia Pro v9.6';
...
const permissionGroups = [
  ...
  { title: 'Administração', items: [['settings.manage', 'Configurações'], ['platform.manage', 'Liberações Master'], ['bank.manage', 'Banco e boletos'], ['system.update', 'Atualizações'], ['audit.view', 'Auditoria'], ['emergency.approve', 'Aprovar emergências']] }
];
...
function UsersResidentsPage({ data, forms, setForm, action, settings, session }) {
  ...
  return <Panel title="Cadastros" subtitle="Moradores, usuários e aprovações separados para não misturar dados de pessoa com acesso ao sistema." icon={<ShieldCheck />}>
...
function SettingsPage({ data, forms, setForm, action, configTab, setConfigTab, enableBrowserNotifications, session }) {
  ...
  const tabs = [['aparencia', 'Aparência'], ['menu', 'Menu'], ['comunicacao', 'Comunicação'], ['reservas', 'Reservas'], ['emergencia', 'Emergência'], ['condominio', 'Condomínio'], ['apps', 'Apps'], ...(session?.role === 'master' ? [['master', 'Master'], ['banco', 'Banco'], ['atualizacoes', 'Atualizações']] : [])];
...
function CentralProPage(props) {
  return <div className="stack"><AppsPage {...props} /><UpdatesPage {...props} /></div>;
}
...
function UpdatesPage({ data, notify, loadAll, action }) {
  ...
  return <Panel title="Central de atualizações" subtitle="Atualize o sistema pelo próprio painel enviando o ZIP oficial com código e token exclusivos." icon={<UploadCloud />}>
    <div className="noticeBox"><b>Como funciona</b><small>O Master envia o ZIP criado oficialmente. O sistema valida produto, código, token interno, checksum, caminhos perigosos e possíveis segredos. Depois pode publicar no GitHub e acionar o deploy do Render quando as variáveis estiverem configuradas.</small></div>
    ...
    <div className="split">
      <section className="updateDrop">
        ...
        <p>O arquivo deve conter <Code>vr-update.json</Code>, arquivo <Code>payload.zip</Code>, código <Code>VRUPD-...</Code>, token único <Code>VRTK-...</Code> e hash do payload.</p>
...
function AppsPage({ settings, session }) {
  ...
  const apps = [
    ['portaria','Portaria APK','Visitantes, encomendas, calendário e OCR.',DoorOpen,'ENABLE_APP_PORTARIA'],
...
function emptyData() { return { dashboard: null, residents: [], users: [], employees: [], shifts: [], messages: [], packages: [], visitors: [], invoices: [], notices: [], reservations: [], finance: [], boletos: [], commonAreas: [], incidents: [], maintenance: [], settings: defaultSettings, emergencyTypes: [], emergencyRequests: [], registrationRequests: [], notifications: [], systemUpdates: [], audit: [], weather: null, updates: [] }; }
