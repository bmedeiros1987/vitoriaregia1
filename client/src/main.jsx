...
const VERSION = import.meta.env.VITE_APP_VERSION || 'Vitória Régia Pro v9.3';
...
function UpdatesPage({ data, notify, loadAll, action }) {
  ...
  return <Panel title="Central de atualizações" subtitle="Atualize o sistema pelo próprio painel enviando o ZIP oficial com código e token exclusivos." icon={<UploadCloud />}>
    ...
    <section className="wide"><h3><ShieldCheck />Atualizações recebidas</h3><Table rows={updates} render={u => <><td><b>{u.title || u.label || u.version || u.update_code}</b><small>{u.update_code}</small><small>{u.source_filename}</small></td><td><Status ok={u.status === 'publicada'}>{u.status || 'validada'}</Status><small>{u.validated_at ? 'validada em ' + date(u.validated_at) : date(u.created_at)}</small></td><td><Code>{u.payload_sha256 || '-'}</Code></td><td className="actions"><button onClick={() => action(`/api/system-updates/${u.id}/notify`, {}, 'Aviso reenviado') }><Bell />Notificar</button><button onClick={() => action(`/api/system-updates/${u.id}/apply`, {}, 'Atualização publicada no GitHub/Render') }><CheckCircle2 />Aplicar</button></td></>} /></section>
    <section className="wide"><h3><Download />Primeira ativação</h3><p>A versão v9.1 ainda não tinha atualização automática completa. Suba esta v9.3 uma vez pelo Mac/GitHub. Depois disso, as próximas atualizações poderão ser enviadas por aqui como ZIP validado.</p></section>
  </Panel>;
}
